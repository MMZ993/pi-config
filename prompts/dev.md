...
4. Load and follow the `verify` skill for the task's planned verification and relevant broader checks. Do not treat pre-existing failures as introduced failures; report them clearly.
5. If verification fails, load and follow the `debugging` skill before changing code again. Do not guess or make random symptom fixes.
6. Mark the task complete in `.agents/PLAN.md` and close its td task only after verification passes.
</task_loop>
...
<stop_conditions>
Stop and ask when the root cause cannot be identified, the task is substantially larger than planned, implementation needs an uncovered decision, or the plan requires revision.
</stop_conditions>
